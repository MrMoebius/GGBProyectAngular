  ...
  template: `
    ...
    <div class="dashboard-wrapper">
      ...
      <div class="stats-grid">
        ...
        <app-stats-card icon="fa-calendar-check" label="Reservas" [value]="reservasCount()" color="var(--neon-purple, #A78BFA)" link="/admin/reservas" />
...
export class AdminDashboardComponent implements OnInit {
  ...
  private buildComandaActivity(comandas: Comanda[]): void {
    ...
    if (pendientes.length > 0) {
      items.push({ icon: 'fa-clock', color: 'var(--warning, #FACC15)', title: `${pendientes.length} comanda${pendientes.length > 1 ? 's' : ''} pendiente${pendientes.length > 1 ? 's' : ''}`, detail: 'Esperando confirmacion', link: '/admin/comandas' });
    }
    if (preparacion.length > 0) {
      items.push({ icon: 'fa-fire-burner', color: 'var(--primary-coral, #FF7F50)', title: `${preparacion.length} comanda${preparacion.length > 1 ? 's' : ''} en preparacion`, detail: 'En cocina ahora', link: '/admin/comandas' });
    }
  ...
  private buildReservaActivity(reservas: ReservasMesa[]): void {
    ...
    if (confirmadaHoy.length > 0) {
      items.push({ icon: 'fa-calendar-check', color: 'var(--success, #22C55E)', title: `${confirmadaHoy.length} reserva${confirmadaHoy.length > 1 ? 's' : ''} hoy`, detail: 'Confirmadas para hoy', link: '/admin/reservas' });
    }
    this.activityItems.update(current => [...current, ...items]);
  }
  ...
  private buildSesionActivity(sesiones: SesionMesa[]): void {
    ...
    if (stale.length > 0) {
      this.activityItems.update(current => [...current, { icon: 'fa-triangle-exclamation', color: 'var(--danger, #EF4444)', title: `${stale.length} sesion${stale.length > 1 ? 'es' : ''} antigua${stale.length > 1 ? 's' : ''}`, detail: 'Abiertas hace mas de 24 horas', link: '/admin/sesiones-mesa' }]);
    }
  }
}
